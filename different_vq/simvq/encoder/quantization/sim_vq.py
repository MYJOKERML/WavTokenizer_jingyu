...
class SimVQ(nn.Module):
    ...
    def forward(self, z, n_q: Optional[int] = None, temp=None, rescale_logits=False, return_logits=False):
...
class SimVQ1D(SimVQ):
    def encode(self, x: torch.Tensor, n_q: Optional[int] = None) -> torch.Tensor:
        return self.forward(x, n_q)[1]
    def decode(self, q_indices: torch.Tensor) -> torch.Tensor:

        quant_codebook = self.embedding_proj(self.embedding.weight)

        z_q = F.embedding(q_indices, quant_codebook)
        z_q = z_q.squeeze(0)
    ...
    def forward(self, z, n_q: Optional[int] = None, temp=None, rescale_logits=False, return_logits=False):
        ...
        if self.sane_index_shape:
            min_encoding_indices = min_encoding_indices.reshape(
                z_q.shape[0], z_q.shape[2], z_q.shape[3])
        # unsqueeze to match the shape of the output of multicodebook
        return z_q, min_encoding_indices.unsqueeze(0).unsqueeze(0), commit_loss.unsqueeze(0).unsqueeze(0)

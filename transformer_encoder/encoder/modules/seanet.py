...
from .mimi_transformer import StreamingTransformer
...
class SEANetEncoder(nn.Module):
    ...
    def __init__(self, channels: int = 1, dimension: int = 128, n_filters: int = 32, n_residual_layers: int = 1,
                 ...
                 pad_mode: str = 'reflect', true_skip: bool = False, compress: int = 2, lstm: int = 2, attn_type: str = 'transformer',
                 num_heads: int = 8, num_layers: int = 4, ff_mult: int = 4):
        super().__init__()
        ...
        assert attn_type in ['lstm', 'transformer'], "attn_type must be either 'lstm' or 'transformer'"
        if lstm and attn_type == 'lstm':
            model += [SLSTM(mult * n_filters, num_layers=lstm)]
        
        if attn_type == 'transformer':
            model += [StreamingTransformer(mult * n_filters, num_heads=num_heads, num_layers=num_layers, dim_feedforward=ff_mult*mult*n_filters)]
...
class SEANetDecoder(nn.Module):
    ...
    def __init__(self, channels: int = 1, dimension: int = 128, n_filters: int = 32, n_residual_layers: int = 1,
                 ...
                 trim_right_ratio: float = 1.0, attn_type: str = 'transformer',
                 num_heads: int = 8, num_layers: int = 4, ff_mult: int = 4):
        super().__init__()
        self.dimension = dimension
        self.channels = channels
